...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				// 2、设置执行spring内部自己定义的和用户自定义的BeanFactoryPostProcessor
				// spring 内部自定义的是：ConfigurationClassPostProcessor
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		//准备一个classloader，设置beanFactory的classLoader为当前context的classLoader
		beanFactory.setBeanClassLoader(getClassLoader());
		//bean表达式解释器，能够获取bean当中的属性在前台页面
		//设置beanFactory的表达式语言处理器，Spring3增加了表达式语言的支持，
		//默认可以使用#{bean.xxx}的形式来调用相关属性值
		beanFactory.setBeanExpressionResolver(new StandardBeanExpressionResolver(beanFactory.getBeanClassLoader()));
		//对象与String类型的转换 <property ref= "">，为beanFactory增加了一个默认的propertyEditor，这个主要是对bean的属性等设置管理的一个工具
		beanFactory.addPropertyEditorRegistrar(new ResourceEditorRegistrar(this, getEnvironment()));

		// Configure the bean factory with context callbacks.
		//添加一个后置处理器
		//能够在bean中获得到各种Aware（*Aware都有器作用）添加BeanPostProcessor
		...
		//意思是如果自定义的Bean中有没有名为“systemProperties”和“systemEnvironment”的Bean
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		...
		//自定义并不仅仅是程序猿自己写的
		//自己写的可以加@Companent也可以不加,是spring自己扫描的
		//如果加了getBeanFactoryPostProcessors()这个地方是得不到的
		//为什么得不到，因为getBeanFactoryPostProcessors()是直接获取一个list，
